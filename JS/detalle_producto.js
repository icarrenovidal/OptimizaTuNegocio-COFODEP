document.addEventListener("DOMContentLoaded", () => {
  ...
  const btnDescontar = document.getElementById("descontarProductoBtn"); // ✅ botón descontar
  ...
  fetch(`./../../PHP/administracion/obtener_detalle_producto.php?id=${productoId}`)
    ...
    .then((data) => {
      ...
      // ======================
      if (btnEditar)
        btnEditar.href = `editar_producto.php?id=${producto.id_producto}`;
      if (btnAgregarLote)
        btnAgregarLote.href = `agregar_stock.php?id=${producto.id_producto}`;
      if (btnDescontar)
        btnDescontar.href = `descontar_producto.php?id=${producto.id_producto}`; // ✅ redirige bien
    })
    .catch((err) => {
